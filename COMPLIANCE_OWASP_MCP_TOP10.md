...
> The list itself is **v0.1 and still a draft** — OWASP places it in "Phase 3: Beta Release and
> Pilot Testing", and its categories are versioned `MCP01:2025`. Mapping against a moving target is
> worth doing and worth saying out loud: the titles below were checked against the published page on
> **2026-08-16**, and if OWASP renumbers or renames anything, this table is wrong until somebody
> re-checks it.
...
| MCP03 | Tool Poisoning | ✅ Mostly | The 10 tools are **statically compiled**, not dynamically fetched or mutable at runtime → cannot be poisoned remotely. Output is flagged untrusted (see MCP06). Zero-width, bidi-override and word-joiner Unicode ("ASCII smuggling" / "Trojan Source") is stripped from returned string values. |
| MCP04 | Software Supply Chain Attacks & Dependency Tampering | ✅ Addressed | Single **static binary**, **distroless non-root** image, minimal audited crate set (no `node_modules`/`pip` chain). Release binaries are built in CI with `--locked` (the audited `Cargo.lock`, not a fresh resolve) and published with SHA-256 checksums on GitHub Releases. Hand-rolled protocol core = tiny dependency surface, no typosquat exposure. |
...
| MCP06 | Intent Flow Subversion | ✅ Addressed (server side) | Every result is wrapped in a structured block flagged `trusted="false"` with provenance; delimiters are escaped so a cell value cannot break out of the block. (The consuming agent/middleware must honor the untrusted flag.) |
