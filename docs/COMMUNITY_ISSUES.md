...
| [#1929](https://github.com/modelcontextprotocol/servers/issues/1929) `INVALID_URL` in Docker | 0 | The real cause is a password containing `@ : / #`; the error names the characters and their encodings. That covers the easy case, where the string fails to parse. The harder one is a password with `@`, which *parses* — the driver takes the last `@` as the separator — and fails at connect time, where the message used to say "check host, port and sslmode" and send you to inspect your firewall. Now the connection path names the password too. *verified* (acceptance: "a password with @ is named as the cause") |
